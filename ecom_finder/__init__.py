    ...
    for item in items:
        ...
        if name == None or price == None:
            ...
            alls.append({
                'name': name.text,
                'price': price.text,
                'url':  'https://amazon.com' + url['href'] if url.has_attr('href') else None,
...
def __get_items_from_flipkart__(text):
    headers = {"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:66.0) Gecko/20100101 Firefox/66.0", "Accept-Encoding":"gzip, deflate", "Accept":"text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8", "DNT":"1","Connection":"close", "Upgrade-Insecure-Requests":"1"}
    query   = "https://www.flipkart.com/search?" + __ulp__.parse.urlencode({'q': text.lower()})
    ...
    for item in soup.select('[data-id]'):
        try:
            alls.append({
                ...
                'price'    : float(item.find_all(text=__re__.compile('₹'))[0].replace('₹', '').replace(',', '')),
                'provider' : 'Flipkart',
                'rating'   : float(item.select('[id*=productRating]')[0].get_text().strip()),
...
def find(keyword, nor=10):
    ...
    return data[(~data['rank'].isna()) & (~data['rating'].isna())].drop_duplicates().sort_values(['rank', 'rating'], ascending = [True, False]).reset_index(drop=True)[:nor][['name', 'price', 'provider', 'url']].rename(columns={
        'name': 'Product Name',
        'price': 'Price',
        'url': 'URL',
